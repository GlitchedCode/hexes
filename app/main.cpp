...
template <> struct glz::meta<AppConfig> {
  ...
  static constexpr auto value =
      glz::object("title", &T::title, "width", &T::width, "height", &T::height);
};
...
static std::string load_example(sol::state &lua,
...
int main() {
  ...
  auto reloader = hexes::fs::FileWatcher::instance().watch_file(
      script, [&lua, &script](hexes::fs::HotReloader &hr) {
        load_example(lua, script);
        return;
      });
